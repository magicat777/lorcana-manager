...
| News job fails: `<source>: 0 items parsed` or an HTTP error | Deliberate loud failure — a 200 page with zero parsed items means a site redesign broke that source's regexes (or a feed got blocked). Sources run in order (`disneylorcana` first, then `comicbook`), each committing as it goes, so items from sources *before* the failed one still landed — but signal enrichment and new-set detection are skipped for that run. Fix the parser (fixture tests: `python -m app.tests.test_fetch_news` in the api pod) or, if ComicBook.com starts 403-ing non-browser agents, remove it from `SOURCES` rather than adding a headless browser. |
