...
- `lorcana_holdings` — top owned cards by value: per-finish day deltas
  ($ and unit-%), foil-premium ratio (✦x: ~1 = play-priced, 2+ =
  collector-priced — answers "trade the foil or the normal?"), and an
  `mv` liquidity proxy (price-moves per 30 nights; single digits =
