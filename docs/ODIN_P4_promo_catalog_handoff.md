...
## Addendum (2026-09-11): PD1 is also incomplete

Jason holds **The Beanstalk – Onward and Upward, `17/PD1 · EN · 14`** (sealed promo wrapper). Lorcast's PD1 stops at 8 cards; this is #17, Set 14 era, base card not yet released. No catalog has it. Provenance unconfirmed (Gen Con 2026 exhibit floor most likely).

Seed from the card face — no base link until Set 14 lands, then backfill:

| Field | Value |
|---|---|
| set_code / collector_number | PD1 / 17 |
| name / version | The Beanstalk / Onward and Upward |
| type | Location |
| ink | Emerald |
| cost | 1 |
| willpower | 6 |
| move_cost | 1 |
| lore | none visible on the face — verify when the base card is published |
| text | THE DISTANT REACHES — Characters get +1 ⚔ and gain Evasive while here. (Only characters with Evasive can challenge them.) |
| illustrator | Alyssa Lee |
| rarity | Promo |

General rule this implies for item 2: promo pools (P*, PD*) are open-ended and Lorcast lags them by weeks to months. The reject list should stay a first-class sync output, not a one-time fix.
