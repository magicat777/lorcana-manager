...
| Grafana dashboard | `http://jason-holt-blade-18-rz09-0484.local:31495/d/lorcana-collection/` (grafana-gitops; datasource `postgres-lorcana` via READ-ONLY role `lorcana_grafana`, password in monitoring Secret `grafana-postgres-lorcana-credentials`). Branded v23: logo header panel loads `/brand/lorcana-logo-sm.png` from the web pod, gold/parchment styling, official per-ink colors on the pivoted copies-by-ink panel, gold foils line on the size chart (from `total_foil`, mig 031), stacked per-set value timeseries (sliced from each snapshot's stored per-set breakdown since v22 — always sums to the total panel; the old today's-quantities × `price_history` form over-valued the past) with series labels rank-prefixed `01 ·`… so the most valuable set stacks at the bottom (v23 — Grafana's long→wide conversion ALPHABETIZES series by label and ignores SQL row order; the label prefix is the only stacking control), market signals as a SPLIT pair (mig 032 tables, §6.3): a CI-only timeseries (per-set median, trigger lines at 0.90 dip / 1.10 momentum) beside a sealed-premium bar gauge (latest price ÷ MSRP per SKU; green ≈ MSRP, yellow ≥1.15, red ≥2) — one merged chart failed twice because daily CI lines in 0.85–1.15 and sparse 1.4–6× sealed levels can't share an axis, Top 20 cards by value with colored per-finish `Δ N` / `Δ ✦` columns (each holding's value change between the two latest daily price snapshots, split normal vs foil — a combined Δ let opposite moves cancel invisibly) plus `✦×` foil-premium ratio (purple 2+ = collector-priced), unit-percent `Δ %` columns (scale-free comparisons), and `Mv N`/`Mv ✦` liquidity proxies (nights of the last ~30 the finish's price moved; TCGplayer market price only ticks on sales, so red single digits = illiquid/stale pricing — true sales counts aren't in Lorcast), "how to read" text headers above the data-quality/movers/Top-20 tables (column meanings + the single-glance takeaway, v18), price data-quality panel (ticks the §6.1 outlier guard flagged, raw vs kept), 7d Core-sets movers table (finish split + CI — normal outrunning foil = play demand), "TCG market context" row (indexed sealed benchmarks per game + collection value line; indexed publisher equities — v20), Fan Content credit footer panel (keep it). JSON in grafana-gitops-live repo; deploy via that repo's `./deploy-lorcana.sh`, which updates BOTH layers: the `grafana-dashboards-shared` ConfigMap (boot-time source of truth — an init-dashboards initContainer copies it over the PVC on every pod start) AND the live PVC file via `kubectl cp -c grafana` (immediate, ~30s rescan). `kubectl cp` alone reverts on the next pod restart — that's how the 2026-09-11 reboot rolled the dashboard back to v1. |
...
### 6.1b CronJob: `lorcana-ask-fetch` — nightly 05:30 PT

Runs `python -m app.jobs.fetch_asks` (mig 036, 2026-09-08): fetches
TCGplayer's **median listed ask** + market price per finish from their public
`mpapi` pricepoints endpoint for owned + want-list cards carrying a
`tcgplayer_id` (~724 cards, 0.3s spacing) into `ask_history`. This is the
second price source: ask sitting **<0.7× market** = the sales-weighted
market price is stale-high, surfaced as ⚠ask-gap flags on card detail,
holdings, and movers (the flag ANNOTATES — CI is never silently altered).
...
spotted-in-store prices. The job's third pass (mig 041) captures the
cross-TCG **market benchmarks**: one sealed product per game via mpapi
(MTG Foundations in-print, Pokemon 151 ETB + OP-01 out-of-print,
Lorcana AotV box) and TCG publisher equities via Yahoo's keyless chart
JSON (HAS, NTDOY, 7832.T, 9766.T, and DIS labeled licensor-only — tiny Lorcana exposure, added 2026-09-16; stooq was JS-walled, and Ravensburger
is private — Lorcana has no ticker). Feeds the dashboard's "TCG market
context" row: everything indexed to 100 at range start, collection
value as the comparison line. Add/retire benchmarks in mig 041.
...
| `collection_log` | Per-card count audit trail (mig 037): every change with source (import w/ import_id, or manual webui/mcp/api edit) + before/after per finish. Backfilled from stored import diffs (horizon 2026-08-04; diffs cap at 500 cards/import; pre-2026-09-10 manual edits unrecoverable). Written by the importer and the collection PUT. |
| `ask_history` | Nightly TCGplayer market + median-ask per finish for owned/want-list cards (mig 036, §6.1b). |
| `card_nicknames` | Agent-writable shorthand registry (mig 036): nick (uppercase) → printing; managed via `lorcana_nickname`, resolved in `lorcana_price_check`. |
| `market_benchmarks` / `market_benchmark_obs` | Cross-TCG context (mig 041): benchmark sealed products (tcgplayer_id) + publisher tickers, one nightly observation each from the ask-fetch job. Basket is migration-owned. |
...
| `collection_snapshots` | Daily + per-import collection state (migs 019/020/031): totals, value, `total_foil` (mig 031 — NULL before 2026-08-22, history not reconstructable), rarity/ink/set/type/cost breakdowns (JSONB). Backfilled rows (from `imports`) carry totals only. Feeds the Stats history charts, Grafana foils line, and (since dashboard v22) the by-set value stack — the `set` breakdown sums to `value_usd` exactly, which is what makes the two value panels agree. |
...
| `duels_games` | The COMPLETE duels.ink ledger (mig 042, per docs/ODIN_duels_ledger_replay_handoff.md): one row per account game from the match-history CSV, keyed on duels `game_id`; PT play-date, result/end-reason/conceded_by, MMR, `opp_kind` (bot\|pvp_ranked\|pvp_casual), mono-ink as `opp_ink_2 NULL`, `is_trivial` (≤2 duels turns, excluded from rates), `match_row_id` link to logged rounds. Fixes the paste-per-game survivorship bias (wins captured 56%, losses 23% at discovery). `your_user_id` never stored; `opp_display_name` stays off shared surfaces. `duels_turns` is duels' own scale (≈ceil(T/2)) — never mix with ODIN T-numbers. |
| `deck_versions` | List snapshots (mig 042): sha1 of sorted (name,count); written by ledger import AND every deck save — overwrite-in-place no longer loses which list a historical match played. |
| `duels_deck_map` | duels deck-id → ODIN deck; seeded with the four verified mappings, importer reports unmapped ids. |
| `duels_replays` | duels-replay-v1 files (mig 043): raw gz in bytea (tiny, backed up nightly — deliberate deviation from the handoff's PVC idea) + parsed metrics jsonb (toss/mulligans/ink drops/quest:challenge/undos/per-card drawn-played-inked-stuck). Parser (`services/duels_replay.py`) honors the format's own `undone` flags, caps decompressed size, warns-not-fails on unknown types; bump `parse_version` + re-run to recompute from raw. Backfill worker `jobs/fetch_replays.py` runs BY HAND (`deploy/jobs/replay-fetch-job.yaml`) at 1 req/s with an identifying UA — /r/ is robots-allowed for ordinary clients, 403/429 aborts loudly, no CronJob until the first backfill proves out. Replays contain the opening hand + deck order: never on shared surfaces. Upload path: `POST /duels/import-replay` (raw gz). Cut-list gains the objective dead-card signal (stuck-in-hand by result) once replays exist. LIVE since 2026-09-20: full 77-replay backfill (0 failures), §4.5 acceptance replay matched every expected value exactly; two format facts learned live — undo frames come as `UNDO:<seq>` as well as `FREE_UNDO:<seq>`, and duels.ink 429s below 1 req/s (worker paces 10s, honors Retry-After, backs off; 403 still stops dead). ~8 rare log types remain warn-only (choice-only undos deliberately NOT counted as action undos). |
...
| Import rejects a promo as `ambiguous name … in set 'NNN'` with a number like `22/P3` | Dreamborn files promos under the BASE set with a compound number (`010` + `22/P3` = promo set P3, collector 22). The matcher parses this since 2026-09-15 (4x Elsa 22/P3 was the find); if a NEW promo-set suffix appears, it just needs a `set_aliases` row. The reject list catching it is the system working — never hand-edit counts around it; fix the cause and re-import. |
| Claude Desktop (or any LAN client) can't reach the MCP / webui by the `.local` name, but the pod is healthy and localhost works | mDNS is serving dead IPv6 addresses. avahi's `use-ipv6=no` only disables the IPv6 TRANSPORT — it still publishes AAAA records over IPv4 mDNS (`publish-aaaa-on-ipv4` defaults to yes), so clients resolve the ULA/privacy addresses that k3s NodePorts (IPv4-only) never answer, churning daily with address rotation. Fix (applied 2026-09-14): `publish-aaaa-on-ipv4=no` under `[publish]` in `/etc/avahi/avahi-daemon.conf`, restart avahi — the journal should then register ONLY the two IPv4 records. Diagnose with `journalctl -u avahi-daemon \| grep Registering`. |
| Grafana dashboard reverted to an old version after a reboot/pod restart | The `init-dashboards` initContainer replays the `grafana-dashboards-shared` ConfigMap over the PVC's `shared/` folder on every pod start — a `kubectl cp`-only deploy lives on the PVC and gets clobbered. Recover: the current JSON is in the grafana-gitops-live repo (git is the real source of truth); run `./deploy-lorcana.sh` there, which patches the ConfigMap AND cp's the live file. The earlier belief "folder configmaps are NOT mounted" was wrong in the way that matters: not mounted in the main container, but fed to the initContainer. |
| Practice losses read 0 in match stats | Fixed 2026-09-18: the loss filter was a stale inline `('1-2','0-2')` missing duels' single-game `'0-1'` (LOSS_RESULTS had it; line 344 predated the constant). Same day: mono-ink opponents rendered `Steel/Steel` — ink-pair expressions now collapse to the single ink. Both patterns also lived in brief.py's meta section. |
| A card's count looks wrong ("I can't find 3 of my 4 Kangas") | `lorcana_card_audit('13/23')` (or ⏱ count history on the card page): shows every count change with its source. Replace-mode import rows MIRROR the scan file — a jump there means the SCAN claimed that count (Dreamborn scanner miscounts are the usual culprit); manual edits show as webui/mcp. If the log doesn't explain it, the count predates 2026-08-04 or an untracked pre-2026-09-10 manual edit. Fix counts via a corrected re-scan (replace mode) so the fix itself is audited. |
| Grafana stacked-timeseries series stack in the wrong order | SQL `ORDER BY` cannot control it: the long→wide conversion ALPHABETIZES series by label and discards row order (proven with a synthetic query, 2026-09-16). Encode the desired order in the label — the by-set value panel uses a zero-padded `dense_rank()` prefix (`01 ·` = largest set, bottom of stack). Don't "clean up" such prefixes. |
| Grafana table sorts wrong after adding a column | Positional `ORDER BY n` in panel SQL: inserting a column shifts the ordinals and the sort silently follows the wrong column (bit 2026-09-08 — adding `✦×` turned the Top 20 into sort-by-foil-ratio, v15→v16). Use a named `ORDER BY "Value"` in panel queries. |
| A mover/price shows an absurd swing (±100%, $1250→$0.25) | Lorcast data artifact, usually on promo printings with near-zero market depth. This is what the movers `min_price` floor, the `Mv` liquidity columns, and the as-of stamp exist for: cross-check `lorcana_card` (moves/30 nights) before believing or quoting it; don't "fix" the history row. |
| Grafana market panel "not updating" | It probably is (diagnosed 2026-09-05): the sealed-premium series only gets a point when a price is hand-logged (event-driven by design — no scrapeable sealed source), and the CI medians move ±0.03/day near 1.0, which reads as flat. Verify the pipeline in order: `price_history` has today's ~3.2k rows (nightly cron), then POST the panel SQL to Grafana's `/api/ds/query` with the glsa token — that exercises the exact rendering path. New CI series appear from want-list membership alone; new sealed series from logging a SKU price. |
| News job fails: `<source>: 0 items parsed` or an HTTP error | Deliberate loud failure — a 200 page with zero parsed items means a site redesign broke that source's regexes (or a feed got blocked). Sources run in order (`disneylorcana`, `comicbook`, `lorcanaplayer`, `lorcanagg`), each committing as it goes, so items from sources *before* the failed one still landed — but signal enrichment and new-set detection are skipped for that run. One caveat on `lorcanagg`: its feed is spam-polluted by design, so "0 items parsed" there can simply mean no Lorcana items passed the guard that day — check the raw feed before declaring the parser broken. Fix the parser (fixture tests: `python -m app.tests.test_fetch_news` in the api pod) or, if a feed starts 403-ing non-browser agents, remove it from `SOURCES` rather than adding a headless browser. |
...
| `lorcana_import_duels_history` | Import the duels.ink account-ledger CSV (complete record; upsert on game_id; two linking passes — lore fingerprint via stored logs, then deck/date/result on match rows — ambiguity reported, never guessed). Ran for real 2026-09-20: 78 games, 0 unresolved card ids, 21 linked, 10 ambiguous; the two pre-v6 Hunny Rescue deck ids remain unmapped pending Jason. |
| `lorcana_duels_coverage` | Capture coverage vs the ledger + unlogged games by PT day (the paste-next list); the survivorship-bias readout. `lorcana_cut_list`/`lorcana_match_stats`/`lorcana_sim_calibration` append a ⚠ win-skew warning while loss coverage < 80% of win coverage. |
| `lorcana_import_duels_log` | Paste a raw duels.ink game log (any of three dialects: spectator copy-paste, timestamped bookmarklet, or logged-in second-person PvP — "You/Opponent" is translated to Player 1/2, redacted opponent draws tolerated): parses winner/turns/lore/plays/impact + undo markers (take-backs REVERT the preceding line since 2026-09-18 — state-snapshot restore, single-depth — so clean undos no longer quarantine a log or double-count plays; three undo surface forms incl. third-person "(free undo)"), auto-detects your seat, infers opponent inks, ranks threats by impact, files a 1-0/0-1 match under the day's duels.ink practice event **for the same deck** (a same-day duels event for a different deck is never reused — added 2026-08-26 after six Hunny games landed under an Elinor event, re-parented as events #23–25), stores the full log (quarantined if internally inconsistent). `overwrite` replaces round+log; identical retries no-op; mvp/dead/tags/threat overrides inline. |
...
| `lorcana_log_event` | Start an event (fuzzy venue + deck-name resolution → returns event id). `date` is OPTIONAL since 2026-09-18 — omitted, the server uses today in PT (agents passing their own UTC-rolled "today" dated three events a day late); a date landing the day AFTER the venue's league night returns a `date_warning`. |
| `lorcana_update_event` | Edit an existing event's header (date, venue, deck, rounds, players, type, notes) — the gap that made the mis-dated events unfixable from chat. Partial: only passed fields change. |
...
| `lorcana_card_audit` | Rogue-count tracker: when/why a card's counts changed — current counts, deck allocation, change log (imports w/ file + note, manual edits w/ source). States its horizon honestly. |
| `lorcana_nickname` | List/register/delete card shorthand ("HAM" → 13/40) — agents register their own instead of assuming. |
| `lorcana_price_check` | Batch price check: list of 'set/number' printings (max 20) → one line each with both finishes (Δ, 7d %, CI, mv, n= per finish), foil ratio, $/legal-week, owned counts; header carries canonical set names + one as-of stamp; accepts registered nicknames; shows median ask with ⚠ask-gap; ✦x suppressed on bulk normals (<$0.50). |
